...
# expression to the language.  This appears as another case in the
# grammar of syntactically well-formed expressions looking like:
#
#     M ::= ... | function f(x : T) : T = M in M

# The idea behind the recursive let `function f(x:T):T' = M in M'` is
# that it introduces a local function name (f) which is visible in
# BOTH the block of code `M'` as well as the body `M'` of the function
# which is named `f`.  In other words, inside the definition of the
# function, the body `M'` has access to both the parameter `x` as and
# may recursively refer to itself via the name `f` that it will be
# bound to.

# The typing rule for a recursive functions is:
# 
#     Γ, f : T->T', x : T ⊢ M : T'    Γ, f : T->T' ⊢ M' : T''
#     -------------------------------------------------------
#     Γ ⊢ function f(x:T) : T' = M in M' : T''

# Notice how the local function name `f:T->T'` is added to the
# environment for checking the term `M : T'` that will be the body of
# `f`, and also in the environment for the type of the result `M':T''`
# of the whole block.

# To implement this extension to the language, add cases for handling
# a recursive let to the following places:
#
#   1. Represent a recursive function (function f(x:T) : T' = M in M') as
#
#          {function: [{name: f, return: T}, {param: x, type: T'}, M, M']}
#
#      where the annotation `return: T` specifies the return type of
#      the function, and the annotation `type: T'` specifies the type
#      of the parameter.
#
...
#      a similar case for pretty-printing a fun expression.
#
